// check-status-fields.js
require('dotenv').config();
const Airtable = require('airtable');

async function checkStatusFields() {
  console.log('🔍 Checking status fields in your Airtable table...\n');
  ...
  try {
    ...
    if (records.length === 0) {
      ...
      if (statusFields.length > 0) {
        ...
        console.log('   No status fields found!');
      }
      ...
      console.log('📋 All fields in your table:');
      fields.forEach(field => {
        console.log(`   • "${field}"`);
      });
      ...
      if (statusFields.length > 0) {
        statusFields.forEach(field => {
          console.log(`   • "${field}" - Current value: "${records[0].fields[field]}"`);
        });
      } else {
        console.log('   No status fields found!');
      }
